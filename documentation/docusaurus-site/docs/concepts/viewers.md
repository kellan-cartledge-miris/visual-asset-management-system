...
| **Miris Stream Viewer**              | 3D       | `.mrx`,  `.usd`, `.usda`, `.usdc`, `.usdz`                                                                                                                                                                    | 1        | No         | Streams a Miris Spatial Streaming asset referenced by a `.mrx` manifest. Requires `MIRIS_STREAMING` and a configured viewer key.                                    |
...
### USD and Miris extensions

| Extension | Default Viewer      | Other Available Viewers          |
| --------- | ------------------- | -------------------------------- |
| `.usd`    | Miris Stream Viewer | Needle USD Viewer (Experimental) |
| `.usda`   | Miris Stream Viewer | Needle USD Viewer (Experimental) |
| `.usdc`   | Miris Stream Viewer | Needle USD Viewer (Experimental) |
| `.usdz`   | Miris Stream Viewer | Needle USD Viewer (Experimental) |
| `.mrx`    | Miris Stream Viewer | --                               |

:::note[USD default viewer depends on the Miris upload feature]
For USD files, the Miris Stream Viewer (priority `0`) is the default only when the `MIRIS_UPLOAD` feature is enabled — it streams the asset if it is already on Miris, or offers a one-click **Stream with Miris** upload otherwise. When `MIRIS_UPLOAD` is disabled, the Needle USD Viewer (Experimental) is the default. The `.mrx` manifest is generated by the [Miris Auto-Upload Pipeline](../pipelines/miris-upload.md).
:::
...
| Miris Stream / Upload Viewers      | Miris Spatial Streaming SDK         | `ALLOWUNSAFEEVAL` (plus `MIRIS_STREAMING` / `MIRIS_UPLOAD`) |
