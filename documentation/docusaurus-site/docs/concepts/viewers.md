...
| **Miris Stream Viewer**              | 3D       | `.mrx`, `.usd`, `.usda`, `.usdc`, `.usdz`                                                                                                                                        | 1        | No         | Streams a Miris Spatial Streaming asset referenced by a `.mrx` manifest. Selecting a USD source file streams it when a `.mrx` exists for the asset. Requires `MIRIS_STREAMING`, `ALLOWUNSAFEEVAL`, and a configured viewer key.                                                                                                                                       |
...
| Miris Stream / Upload Viewers      | Miris Spatial Streaming SDK (`@miris-inc/three`) | `ALLOWUNSAFEEVAL` (plus `MIRIS_STREAMING` / `MIRIS_UPLOAD`) |

:::warning[Content Security Policy]
Enabling `ALLOWUNSAFEEVAL` adds `unsafe-eval` to the Content Security Policy `script-src` directive. This is required by the SuperSplat Editor and by WASM loaders for OpenCascade and Needle USD. Review this setting with your organization's security team before enabling. Set `app.webUi.allowUnsafeEvalFeatures` to `true` in the CDK `config.json` to enable.
:::
