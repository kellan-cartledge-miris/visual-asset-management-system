...
| **Miris Stream Viewer**              | 3D       | `.mrx`                                                                                                                                                                  | 1        | No         | Streams a Miris Spatial Streaming asset referenced by a `.mrx` manifest. Requires `MIRIS_STREAMING` and a configured viewer key.                                    |
...
| Extension | Default Viewer            | Other Available Viewers          |
| --------- | ------------------------- | -------------------------------- |
| `.usd`    | Miris Upload / USD Viewer | Needle USD Viewer (Experimental) |
| `.usda`   | Miris Upload / USD Viewer | Needle USD Viewer (Experimental) |
| `.usdc`   | Miris Upload / USD Viewer | Needle USD Viewer (Experimental) |
| `.usdz`   | Miris Upload / USD Viewer | Needle USD Viewer (Experimental) |
| `.mrx`    | Miris Stream Viewer       | --                               |

:::note[USD default viewer depends on the Miris upload feature]
The Miris Upload / USD Viewer (priority `0`) is the default for USD files only when the `MIRIS_UPLOAD` feature is enabled. When it is disabled, the Needle USD Viewer (Experimental) is the default. The `.mrx` manifest is generated by the [Miris Auto-Upload Pipeline](../pipelines/miris-upload.md).
:::
