...
def get_miris_viewer_key():
    """Get the Miris Spatial Streaming viewer key from the environment.

    The frontend uses this in the Miris streaming viewer plugin; an empty string
    means the deployment is "not configured" and the plugin stays inactive. The
    key is deployment-wide and injected by the config Lambda builder as the
    MIRIS_VIEWER_KEY env var only when the MIRIS_STREAMING feature is enabled.

    Returns:
        The Miris viewer key or empty string if not configured
    """
    return os.getenv("MIRIS_VIEWER_KEY", "")
...
def handle_get_request(event):
    ...
    try:
        response_model = SecureConfigResponseModel(
            ...
            mirisViewerKey=get_miris_viewer_key()
        )
        logger.info("Success")
